...
class ManifoldFMLitModule(pl.LightningModule):
    ...
    def sample(self, n_samples, device, x0=None):
        ...
        if not eval_projx and not local_coords:
            ...
            x1 = odeint(
                ...
                t=torch.linspace(0, 1, 1).to(device),
                atol=self.cfg.model.atol,
                rtol=self.cfg.model.rtol,
        ...
        try:
            ...
            with torch.inference_mode(mode=False):
                ...
                def odefunc(t, tensor):
                    ...
                    vecfield = lambda x: self.vecfield(t, torch.tensor([0.]).to(tensor), x)
    ...
    def rfm_loss_fn(self, batch: torch.Tensor):
        ...
        N = x1.shape[0]
        # Sample two time steps (r, t)
        t, r = self.sample_tr(N, device=x1.device)
        ...
        if isinstance(self.manifold, Mesh):
            ...
            def cond_path(x0_i, x1_i, t_i):
                path = geodesic(self.manifold, x0_i, x1_i)
                x_t_i, u_t_i = jvp(path, (t_i,), (torch.ones_like(t_i).to(t_i),))
                return x_t_i, u_t_i
            
            x_t, u_t = vmap(cond_path)(x0, x1, t)
            # x_t and u_t should already have the correct shape (N, dim)
        
        # Compute u and du_dt using JVP
        def u_fn(t, r, x):
            return self.vecfield(t, r, x)
        
        dt_dt = torch.ones_like(t)
        dr_dt = torch.zeros_like(r)
        
        # JVP with tangent vector (dt_dt, dr_dt, u_t) for (t, r, x)
        u, du_dt = jvp(
            u_fn,
            (t, r, x_t),
            (dt_dt, dr_dt, u_t)
        ...
        u_tgt = u_t - torch.clamp(t - r, min=0.0, max=1.0).reshape(-1, 1) * du_dt
        u_tgt = u_tgt.detach()  # stop gradient
        
        # Compute loss with adaptive weighting
        diff = u - u_tgt
        loss = self.manifold.inner(x_t, diff, diff)  # (N,)
        
        return loss.mean() / self.dim

    def sample_tr(self, batch_size, device, time_dist=('uniform',), flow_ratio=0.5):
        if time_dist[0] == 'uniform':
            samples = np.random.rand(batch_size, 2)
        
        elif time_dist[0] == 'lognorm':
            mu, sigma = time_dist[-2], time_dist[-1]
