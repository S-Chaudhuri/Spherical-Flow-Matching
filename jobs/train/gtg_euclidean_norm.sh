#!/bin/bash
#SBATCH --job-name=gtg_euc
#SBATCH --time=00:15:00            # Adjust as needed
...
#SBATCH --array=1-24             
#SBATCH --output=slurm_output/gtg/euclidean/norm_array_%A_%a.out 
#SBATCH --error=slurm_output/gtg/euclidean/norm_array_%A_%a.err  

PROJECT_ROOT="$HOME/Spherical-Flow-Matching"
TASKS_FILE="$PROJECT_ROOT/tasks/gtg_euclidean_norm.txt"
...
mkdir -p "$PROJECT_ROOT/metrics_links/gtg/euclidean"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    LOCAL_SYMLINK_DIR="$PROJECT_ROOT/metrics_links/gtg/euclidean"
...
# If the exit code is not 0, the job failed.
    ...
    mkdir -p "$PROJECT_ROOT/failed_tasks/gtg/euclidean"
    cp "$PROJECT_ROOT/slurm_output/gtg/euclidean/norm_array_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out" "$PROJECT_ROOT/failed_tasks/gtg/euclidean/"
    cp "$PROJECT_ROOT/slurm_output/gtg/euclidean/norm_array_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err" "$PROJECT_ROOT/failed_tasks/gtg/euclidean/"
    
    echo "Logs copied to failed_tasks/gtg/euclidean/ directory."
fi
