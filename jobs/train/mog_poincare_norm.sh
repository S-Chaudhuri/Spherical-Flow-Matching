...
#SBATCH --error=/slurm_output/tch-shared/%u/logs/mog/poincare/norm_array_%A_%a.err  

# Create necessary directories
mkdir -p slurm_output/mog/poincare 
mkdir -p failed_tasks/mog/poincare
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    LOCAL_SYMLINK_DIR="metrics_links/mog/poincare"
...
# If the exit code is not 0, the job failed.
    ...
    cp "/slurm_output/mog/poincare/norm_array_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out" /failed_tasks/mog/poincare/
    cp "/slurm_output/mog/poincare/norm_array_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err" /failed_tasks/mog/poincare/
