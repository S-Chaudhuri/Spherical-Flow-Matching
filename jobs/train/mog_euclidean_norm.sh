...
TASKS_FILE="$PROJECT_ROOT/tasks/mog_euclidean_norm.txt"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    LOCAL_SYMLINK_DIR="$PROJECT_ROOT/metrics_links/mog/euclidean"
...
# If the exit code is not 0, the job failed.
    ...
    mkdir -p "$PROJECT_ROOT/failed_tasks/mog/euclidean"
    cp "$PROJECT_ROOT/slurm_output/mog/euclidean/norm_array_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out" "$PROJECT_ROOT/failed_tasks/mog/euclidean/"
    cp "$PROJECT_ROOT/slurm_output/mog/euclidean/norm_array_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err" "$PROJECT_ROOT/failed_tasks/mog/euclidean/"
